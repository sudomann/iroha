...
namespace integration_framework {
  ...
  FakePeerBehaviour::~FakePeerBehaviour() {
    absolve();
  }

  void FakePeerBehaviour::adopt(const std::shared_ptr<FakePeer> &fake_peer) {
    ...
    log_ =
        logger::log(getName() + " of fake peer at " + fake_peer->getAddress());
    // subscribe for all messages
    subscriptions_.emplace_back(
        getFakePeer().get_mst_states_observable().subscribe(
            [this](const auto &message) { this->processMstMessage(message); }));
    subscriptions_.emplace_back(
        getFakePeer().get_yac_states_observable().subscribe(
            [this](const auto &message) { this->processYacMessage(message); }));
    subscriptions_.emplace_back(
  ...
  void FakePeerBehaviour::absolve() {
  ...
  FakePeer& FakePeerBehaviour::getFakePeer() {
